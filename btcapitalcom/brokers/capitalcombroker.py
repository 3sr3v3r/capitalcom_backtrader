...
###############################################################################
#
# 2023: Jelle Bloemsma, backtrader broker functionality for Capital.com
...
class CapitalcomCommInfo(CommInfoBase):
...
class MetaCapitalcomBroker(BrokerBase.__class__):
    def __init__(cls, name, bases, dct):
        '''Class has already been created ... register'''
        # Initialize the class
        super(MetaCapitalcomBroker, cls).__init__(name, bases, dct)
        capitalcomstore.CapitalcomStore.BrokerCls = cls


class CapitalcomBroker(with_metaclass(MetaCapitalcomBroker, BrokerBase)):
    '''Broker implementation for Capitalcom.

    This class maps the orders/positions from Capitalcom to the
    ...
    params = (
        ('use_positions', True),
        ('commission', CapitalcomCommInfo(mult=1.0, stocklike=False)),
    )

    def __init__(self, **kwargs):
        super(CapitalcomBroker, self).__init__()

        self.o = capitalcomstore.CapitalcomStore(**kwargs)
    ...
    def start(self):
        super(CapitalcomBroker, self).start()
        ...
        if self.p.use_positions:
            for p in self.o.get_positions():
                ...
                size = float(p['position']['size']) / float(p['position']['leverage'])
                price = (float(p['position']['level']))
    ...
    def stop(self):
        super(CapitalcomBroker, self).stop()
        self.o.stop()

    def getcash(self):
        # This call cannot block if no answer is available from Capitalcom
